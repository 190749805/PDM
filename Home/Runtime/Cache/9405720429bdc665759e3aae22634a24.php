...
<script src="__PUBLIC__/Js/Info/info15.js"></script>
...
<form action="__URL__/commitInfo15" method="post">
    <input type="hidden" value="<?php echo ($va[0]['id']); ?>" name="hidden"/>
    <div class="center">
        <p class="title">编写教材说明</p>
        <table class="info_table">
            <tr>
                <th>教材名称</th><th>出版时间</th><th>类别</th><th>出版社</th><th>本人承担部分</th>
            ...
            <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                    ...
                    <th><?php echo ($vo["qksm"]); ?></th><th><?php echo ($vo["num"]); ?></th>
                    <th>
                        <a href="__URL__/info15/id/<?php echo ($vo["id"]); ?>">修改</a>
                        <a href="__URL__/deleteInfo15/id/<?php echo ($vo["id"]); ?>">删除</a>
                    </th>
                </tr><?php endforeach; endif; else: echo "" ;endif; ?>
        </table>
        <br/>
        <input class="btn save15"  type="submit" value="保存"/>
        <br/>
        <br/>
        <p class="name">教材有无情况<select name="qk">
        ...
        <p class="name hid">教材名称：<input id="mc" value="<?php echo ($va[0]['mc']); ?>" type="text" name="mc" />
        ...
        <p class="name hid">类别：<select name="lb">
        ...
        <p class="name hid">本人承担部分：<select name="cdbf">
        ...
        <p class="name hid">情况说明(例:编写XX章，XX字)：<b class="b-tishi" id="b-sm"></b><br/>
            编写<input id="sm" value="<?php echo ($va[0]['qksm']); ?>" type="text" name="qksm"/>
            章，共<input type="text" name="num" size="5" value="<?php echo ($va[0]['num']); ?>">千字

            <b class="b-tishi" id="b-num"></b>
        </p>
        <?php if($va[0]['id'] == null): ?><p class="name"><button class="btn save15 hid">增加</button></p>
            <?php else: ?><p class="name"><button class="btn save15">修改</button></p><?php endif; ?>
        <div class="daohang">
            <input type="button" onclick="window.location.href='__URL__/info14'" class="btn" value="上一页">
            <?php $__FOR_START_18759__=1;$__FOR_END_18759__=17;for($i=$__FOR_START_18759__;$i < $__FOR_END_18759__;$i+=1){ ?><input type="button" onclick="window.location.href='__URL__/info<?php echo ($i); ?>'" class="btn" value="<?php echo ($i); ?>"><?php } ?>
            <input type="button" onclick="window.location.href='__URL__/info16'" class="btn" value="下一页">
        </div>
        <br/>
</form>
</div>
