...
<script src="__PUBLIC__/Js/Info/info14.js"></script>
...
<form action="__URL__/commit_info14" method="post">
<input type="hidden" value="<?php echo ($va[0]['id']); ?>" name="hidden">
<div class="center">
<p class="title">发明专利</p>
<table class="info_table">
		<tr>
			<th>授权时间</th><th>专利名称</th><th>专利类型</th><th>是否授权</th>
			<th>授权专利号</th><th>操作</th>
		</tr>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
		<th><?php echo ($vo["tgsj"]); ?></th><th><?php echo ($vo["mc"]); ?></th><th><?php echo ($vo["lx"]); ?></th><th><?php echo ($vo["sq"]); ?></th>
		<th><?php echo ($vo["zlh"]); ?></th>
		<th>
			<a href="__URL__/info14/id/<?php echo ($vo["id"]); ?>">修改</a>
			<a href="__URL__/delete_info14/id/<?php echo ($vo["id"]); ?>">删除</a>
		</th>
	...
	<button class="btn save14">保存</button>
	<p class="name">专利有无情况:
        <select name="qk">
	...
	<p class="name hid">专利名称：<input id="mc" value="<?php echo ($va[0]['mc']); ?>" type="text" name="mc">
	<b class="b-tishi" id="b-mc"></b>
	</p>
	<p class="name hid">专利类型：<select name="slx">
		<?php if(is_array($lx)): $i = 0; $__LIST__ = $lx;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lx): $mod = ($i % 2 );++$i; echo ($lx); endforeach; endif; else: echo "" ;endif; ?>
		<?php if(is_array($lxa)): $i = 0; $__LIST__ = $lxa;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$lxa): $mod = ($i % 2 );++$i; echo ($lxa); endforeach; endif; else: echo "" ;endif; ?>
		</select>
	</p>
	
	<!--<p class="name hid">是否授权：<select name="ssq">
		<?php if(is_array($sq)): $i = 0; $__LIST__ = $sq;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sq): $mod = ($i % 2 );++$i; echo ($sq); endforeach; endif; else: echo "" ;endif; ?>
		<?php if(is_array($sqa)): $i = 0; $__LIST__ = $sqa;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$sqa): $mod = ($i % 2 );++$i; echo ($sqa); endforeach; endif; else: echo "" ;endif; ?>
		</select>
	</p>-->
    <p class="name hid">授权时间:
        <select name="tgsjy">
	...
	<p class="name hid">授权专利号:<input id="zlh" value="<?php echo ($va[0]['zlh']); ?>" type="text" name="zlh">
	<b class="b-tishi" id="b-zlh"></b>
	</p>
	<?php if($va[0]['id'] == null): ?><p class="name"><button class="btn save14 hid">增加</button></p>
	<?php else: ?><p class="name"><button class="btn save14">修改</button></p><?php endif; ?>
	<div class="daohang">
			<input type="button" onclick="window.location.href='__URL__/info13'" class="btn" value="上一页">
				<?php $__FOR_START_6930__=1;$__FOR_END_6930__=17;for($i=$__FOR_START_6930__;$i < $__FOR_END_6930__;$i+=1){ ?><input type="button" onclick="window.location.href='__URL__/info<?php echo ($i); ?>'" class="btn" value="<?php echo ($i); ?>"><?php } ?>
			<input type="button" onclick="window.location.href='__URL__/info15'" class="btn" value="下一页">
		</div>
		<br/>
	</form>
</div>
